...
import { Dashboard, Cashier, Inventory, Kitchen, Settings, Table, Bar} from "./svg";
...
export default function AdminNavbar({ user }: { user: User }) {
  ...
  return (
    <aside className="w-64 bg-violet-500 text-white min-h-screen flex flex-col">
      ...
      <nav className="flex-1 p-4 space-y-2">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={`block px-3 py-2 rounded ${pathname === item.href
      ...
      <div className="p-4 border-t border-violet-600">
        <form action={logout}>
          <button
            type="submit"
            className="w-full text-left px-3 py-2 rounded text-sm bg-violet-700 hover:bg-violet-600 transition"
          >
            Cerrar Sesión
          </button>
        </form>
