...
const navItems = [
  { name: "Panel", href: "/dashboard", logo: <Dashboard /> },
  { name: "Mesas", href: "/tables", logo: <Table /> },
  { name: "Cocina", href: "/kitchen", logo: <Kitchen /> },
...
export default function AdminSidebar({ user }: { user: User }) {
  ...
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 h-16 bg-violet-500 text-white flex items-center justify-between px-4 z-40 md:hidden">
        <button
          onClick={() => setIsOpen(true)}
          className="p-2 rounded hover:bg-violet-600 transition"
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:static inset-y-0 left-0 w-64 bg-violet-500 text-white flex flex-col z-50 transform transition-transform duration-300 ease-in-out md:transform-none ${
      ...
      >
        ...
        <button
          onClick={() => setIsOpen(false)}
          className="absolute top-4 right-4 p-2 rounded hover:bg-violet-600 transition md:hidden"
        ...
        {/* Logo & User */}
        <div className="p-4 border-b border-violet-600 pt-16 md:pt-4">
        ...
        <div className="p-4 border-t border-violet-600">
          <form action={logout}>
            <button
              type="submit"
              className="w-full text-left px-3 py-2 rounded-lg text-sm bg-violet-700 hover:bg-violet-600 transition"
            >
              Cerrar Sesión
            </button>
          </form>
